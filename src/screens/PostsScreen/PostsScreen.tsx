...
const PostsScreen: React.FC = () => {
    ...
    // console.log('adaddadadadaad=vbd-----c-0-----', item?.user_info);
    ...
    const [images, setImages] = useState<[]>(allPhotos??[]);

    useEffect(() => {
        if (data) {
            const clickedImage = allPhotos.find(i => i.id === data?.data?.id);
            if (clickedImage) {
                setImages([
                    clickedImage,
                    ...allPhotos.filter(i => i.id !== data?.data.id),
                ]);
            } else {
                setImages([clickedImage, ...allPhotos]);
            }
        }
    }, [allPhotos]);
